...
if __name__ == '__main__':
    ...
    """算法5：EEI（Explosive Immunization）"""
    start = time.time()
    # 算法超时统计
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future = executor.submit(helpers.EEI, names[name_id], C, name_id)
        try:
            # 在timeout秒内等待结果
            x, y, _, G = future.result(timeout=timeout)
            # 如果在超时时间内完成，输出运行时间
            helpers.Elapsed_Time(start, 'EEI', outputstr_list)
            
            """保存拆解后的残差网络"""
            if isSaveResidualGraph:
                helpers.Save_Residual_Graph_Components(G, names[name_id], 'EEI')
            ...
            # 设置自定义的文件名
            filename = f'AttackCurve/{names[name_id]}_EEI.txt'
            # 将字符串保存到指定文件
