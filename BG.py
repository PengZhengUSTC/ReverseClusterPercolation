...
if __name__ == '__main__':
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future = executor.submit(ExperimentCode.Bridgeness, G_ig)
        try:
            # 在timeout秒内等待结果
            x, y, _, G_ig_new = future.result(timeout=timeout)
            # 如果在超时时间内完成，输出运行时间
            helpers.Elapsed_Time(start, 'BG', outputstr_list)
            # 将igraph转换成nx.graph
            G = helpers.convert_igraph_to_nx(G_ig_new)

            """保存拆解后的残差网络"""
            if isSaveResidualGraph:
                helpers.Save_Residual_Graph_Components(G, names[name_id], 'BG')
            ...
            # 设置自定义的文件名
            filename = f'AttackCurve/{names[name_id]}_BG.txt'
            # 将字符串保存到指定文件
