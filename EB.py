...
if __name__ == '__main__':
    ...
    """算法2：EB（edge_betweenness_centrality）"""
    G_ig = helpers.convert_nx_to_igraph(G_origin)

    start = time.time()
    # 算法超时统计
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future = executor.submit(ExperimentCode.Edge_betweeness, G_ig)
        try:
            # 在timeout秒内等待结果
            x, y, _, G_ig_new = future.result(timeout=timeout)
            # 如果在超时时间内完成，输出运行时间
            helpers.Elapsed_Time(start, 'EB', outputstr_list)
            # 将igraph转换成nx.graph
            G = helpers.convert_igraph_to_nx(G_ig_new)

            """保存拆解后的残差网络"""
            if isSaveResidualGraph:
                helpers.Save_Residual_Graph_Components(G, names[name_id], 'EB')
            ...
            # 设置自定义的文件名
            filename = f'AttackCurve/{names[name_id]}_EB.txt'
            # 将字符串保存到指定文件
