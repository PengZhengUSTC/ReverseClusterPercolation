...
int main(int argc, char const *argv[])
{
  ...
  double dismantling_threshold = atof(argv[4]);   //拆解阈值
  double k_ebpd   = atof(argv[5]);   //原网络边数和节点数的比值
  
  cout<<"N="<<VertexNumber<<" "<<argv[2]<<" M="<<EdgeNumber<<" "<<argv[3]<<" dismantling_threshold="<<dismantling_threshold<<" "<<argv[4]<<" k_ebpd="<<k_ebpd<<" "<<argv[5]<<endl;
  ...
  */
  ...
  string degreefile=graphname+".degree";                      //degree rank
  system.DegreeRank(degreefile);
  int Degree0=1000;   /* Delete vertices whose active degree is larger than Degree0.
...
void FVS::AttackEffect(string& attackfile)
{
  ...
  output<< (1.0e0*num_empty)/(1.0e0*VertexNumber) << '\t'
	<< num_empty << '\t'
  << (1.0e0*num_active_edge)/(1.0e0*EdgeNumber) << '\t'
  << num_active_edge << '\t'
